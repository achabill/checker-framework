...
public class DivByZeroTransfer extends CFTransfer {
    ...
    /**
     * Assuming that a simple comparison (lhs `op` rhs) returns true, this function should refine
     * what we know about the left-hand side (lhs). (The input value "lhs" is always a legal return
     * value, but not a very useful one.)
     *
     ...
     * </pre>
     *
     * the comparison "y != 0" causes us to learn the fact that "y is not zero" inside the body of
     * the if-statement. This function would be called with "NE", "top", and "zero", and should
     * return "not zero" (or the appropriate result for your lattice).
     *
     * <p>Note that the returned value should always be lower in the lattice than the given point
     * for lhs. The "glb" helper function below will probably be useful here.
     *
    ...
    private AnnotationMirror refineLhsOfComparison(
        ...
        switch (operator) {
            case EQ:
                return glb(lhs, rhs);
            case NE:
                if (AnnotationUtils.areSameByClass(rhs, Zero.class)) {
                    return glb(lhs, reflect(NonZero.class));
                }
                if (AnnotationUtils.areSameByClass(rhs, NonZero.class)) {
            ...
            case GT:
                ...
                break;
                // GTE and LTE are not interesting.
        }
        return lhs;
    }

    /**
     * For an arithmetic expression (lhs `op` rhs), compute the point in the lattice for the result
     * of evaluating the expression. ("Top" is always a legal return value, but not a very useful
     * one.)
     *
     * <p>For example,
     *
    ...
    private AnnotationMirror arithmeticTransfer(
        ...
        switch (operator) {
            case PLUS:
            case MINUS:
                if (AnnotationUtils.areSameByClass(lhs, Zero.class)) {
                    return rhs;
                }
                if (AnnotationUtils.areSameByClass(rhs, Zero.class)) {
                    return lhs;
    ...
    private AnnotationMirror reflect(Class<? extends Annotation> qualifier) {
        return AnnotationUtils.fromClass(
